...
if [ "$build_type" == "d" ]; then
    build_type="debug_clang"
elif [ "$build_type" == "r" ]; then
    build_type="release_clang"
elif [ "$build_type" == "p" ]; then 
    build_type="profile"
fi

# build_type이 release나 debug가 아니라면 종료
if [ "$build_type" != "release_clang" ] && [ "$build_type" != "debug_clang" ] && [ "$build_type" != "profile" ]; then
...
mkdir -p ${dest_dir}/pl_engine/lib/

export CUBRID=${dest_dir}
cmake --preset ${build_type} -DCMAKE_INSTALL_PREFIX=${dest_dir}
